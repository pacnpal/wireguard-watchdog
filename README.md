...
6. **Failure simulation (hard path)**
   - From SSH: `wg-quick down wg0` to fully tear down the interface.
   - Wait one cron interval (or click **Test Now** to force).
   - Expect `wg syncconf` to be skipped (interface absent), then
     `wg-quick down wg0: failed (continuing)` and `wg-quick up wg0: ok`.
   - Verify `wg show wg0` reports the interface back up.
